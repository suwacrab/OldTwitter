class TweetViewer {
    constructor(user, tweetData) {
        ...
        this.container = createModal(
            html`
                ...
                <div class="timeline-more center-text" hidden>
                    ${LOC.load_more.message}
                </div>
                <div class="retweets-more center-text" hidden>
                    ${LOC.load_more.message}
                </div>
                <div class="retweets_with_comments-more center-text" hidden>
                    ${LOC.load_more.message}
                </div>
                <div class="likes-more center-text" hidden>
                    ${LOC.load_more.message}
                </div>
            `,
            "tweet-viewer",
            () => {
                this.close();
                if (location.pathname + location.search !== previousLocation)
        ...
        history.pushState(
            {},
            null,
            `/${tweetData.user.screen_name}/status/${this.id}`
        );
        ...
        this.timelineElement =
            this.container.getElementsByClassName("timeline")[0];
        this.moreBtn =
            this.container.getElementsByClassName("timeline-more")[0];
    ...
    async restorePageData() {
        ...
        if (this.pageData[path]) {
            ...
            this.retweetCommentsCursor =
                this.pageData[path].retweetCommentsCursor;
            this.mainTweetLikers = this.pageData[path].mainTweetLikers;
            this.seenReplies = [];
            this.tweets = [];
            this.container.getElementsByClassName("timeline-more")[0].hidden =
                !this.cursor;
            ...
            if (id) {
                setTimeout(() => {
                    let tweet = this.container.querySelector(
                        `div.tweet[data-tweet-id="${id}"]`
                    );
                    if (tweet) {
    ...
    updateSubpage() {
        ...
        let rtwDiv = document.getElementsByClassName(
        ...
        let rtwMore = document.getElementsByClassName(
            "retweets_with_comments-more"
        )[0];
    ...
    async updateReplies(id, c) {
        let tvl = this.container.getElementsByClassName(
            "tweet-viewer-loading"
        )[0];
        if (!c) {
            tvl.hidden = false;
        ...
        let tl, tweetLikers;
        try {
            let [tlData, tweetLikersData] = await Promise.allSettled([
                API.tweet.getRepliesV2(id, c),
                API.tweet.getLikers(id, undefined, 40),
            ]);
            if (!tlData.value) {
                if (!c) {
                    tvl.hidden = true;
                    document.getElementsByClassName(
                        "timeline"
                    )[0].innerHTML = html`<span
                        style="color:var(--almost-black)"
                        >${tlData.reason}</span
                    >`;
                }
                this.cursor = undefined;
            ...
            this.loadingNewTweets = false;
            document.getElementsByClassName("timeline-more")[0].innerText =
                LOC.load_more.message;
        } catch (e) {
            document.getElementsByClassName("timeline-more")[0].innerText =
                LOC.load_more.message;
        ...
        if (vars.slowLinkColorsInTL) {
            ...
            for (let i in tl.list) {
                let t = tl.list[i];
                if (t.type === "tweet" || t.type === "mainTweet") {
                    if (!tlUsers.includes(t.data.user.id_str))
                        tlUsers.push(t.data.user.id_str);
    ...
    async updateLikes(id, c) {
        ...
        if (!c) {
            ...
            h1.innerText = vars.heartsNotStars
                ? LOC.liked_by.message
                : LOC.favorited_by.message;
    ...
    async updateRetweets(id, c) {
        ...
        if (!c) {
            ...
            tweet.style.borderRadius = "5px";
            let h1 = document.createElement("h1");
            h1.innerHTML = html`${LOC.retweeted_by.message} (<a
                    href="/${tweetData.user
                        .screen_name}/status/${id}/retweets/with_comments"
                    >${LOC.see_quotes.message}</a
                >)`;
            h1.className = "cool-header";
            retweetDiv.appendChild(h1);
            h1.getElementsByTagName("a")[0].addEventListener(
                "click",
                async (e) => {
                    e.preventDefault();
                    history.pushState(
                        {},
                        null,
                        `/${tweetData.user.screen_name}/status/${id}/retweets/with_comments`
                    );
                    this.updateSubpage();
                    this.mediaToUpload = [];
                    this.excludeUserMentions = [];
        ...
        for (let i in tweetRetweeters) {
            ...
            retweetElement.innerHTML = html` <div>
                    <a href="/${u.screen_name}" class="following-item-link">
                        <img
                            src="${u.default_profile_image &&
                        ...
                        <div class="following-item-text">
                            <span class="tweet-header-name following-item-name"
                                >${escapeHTML(u.name)}</span
                            ><br />
                            <span class="tweet-header-handle"
                                >@${u.screen_name}</span
                            >
                            ${u.followed_by
                ...
                <div>
                    <button
                        class="following-item-btn nice-button ${u.following
                            ? "following"
                            : "follow"}"
                    >
                        ${u.following
    ...
    async updateRetweetsWithComments(id, c) {
        ...
        if (!c) {
            retweetDiv.innerHTML = "";
            let h1 = document.createElement("h1");
            h1.innerHTML = html`${LOC.quote_tweets.message} (<a
                    href="/${tweetData.user.screen_name}/status/${id}/retweets"
                    >${LOC.see_retweets.message}</a
                >)`;
            h1.className = "cool-header";
            retweetDiv.appendChild(h1);
            h1.getElementsByTagName("a")[0].addEventListener(
                ...
                async (e) => {
                    ...
                    history.pushState(
                        {},
                        null,
                        `/${tweetData.user.screen_name}/status/${id}/retweets`
                    );
                    this.updateSubpage();
                    this.mediaToUpload = [];
                    this.excludeUserMentions = [];
    ...
    async appendComposeComponent(container, replyTweet) {
        ...
        if (LOC.reply_to.message.includes("$SCREEN_NAME$")) {
            replyMessage = LOC.reply_to.message.replace(
                "$SCREEN_NAME$",
        ...
        el.innerHTML = html` <div class="new-tweet-view box">
            <img width="35" height="35" class="tweet-avatar new-tweet-avatar" />
            <span class="new-tweet-char" hidden
                >${localStorage.OTisBlueVerified ? "0" : "0/280"}</span
            >
            <textarea
                class="new-tweet-text"
            ...
            <div class="new-tweet-focused" hidden>
                <span
                    class="new-tweet-emojis"
                    title="${LOC.emoji.message}"
                ></span>
                ${mentions.length > 0
                    ? html`<span
                          class="new-tweet-mentions"
                          title="${LOC.mentions.message}"
                      ></span>`
                    : ""}
                <div class="new-tweet-media-cc">
                    <div class="new-tweet-media-c"></div>
                </div>
                <button
        ...
        container.append(el);
        document.getElementsByClassName("new-tweet-avatar")[0].src = `${
            this.user.default_profile_image && vars.useOldDefaultProfileImage
                ? chrome.runtime.getURL(
                      `images/default_profile_images/default_profile_${
                          Number(this.user.id_str) % 7
                      }_normal.png`
                  )
                : this.user.profile_image_url_https
        }`.replace("_normal", "_bigger");
        document
            .getElementsByClassName("new-tweet-view")[0]
            .addEventListener("click", async () => {
                ...
                document
                    .getElementsByClassName("new-tweet-text")[0]
                    .classList.add("new-tweet-text-focused");
                document
                    .getElementsByClassName("new-tweet-media-div")[0]
                    .classList.add("new-tweet-media-div-focused");
            });
        if (mentions.length > 0) {
            for (let i = 0; i < mentions.length; i++) {
                let u = Object.values(this.users).find(
                    (u) => u.screen_name === mentions[i]
                );
                if (!u) {
                    if (mentions[i] === this.user.screen_name) {
                        ...
                        mentions[i] === pageUser.screen_name
                    ) {
                        u = pageUser;
                    } else {
                        try {
            ...
            document
                ...
                .addEventListener("click", async () => {
                    let modal = createModal(html`
                        <div
                            id="new-tweet-mentions-modal"
                            style="color:var(--almost-black)"
                        >
                            <h3 class="nice-header">
                                ${LOC.replying_to.message}
                            </h3>
                            <div class="new-tweet-mentions-modal-item">
                                <input
                                    type="checkbox"
                                    id="new-tweet-mentions-modal-item-${replyTweet
                                        .user.screen_name}"
                                    checked
                                    disabled
                                />
                                <label
                                    for="new-tweet-mentions-modal-item-${replyTweet
                                        .user.screen_name}"
                                    >${replyTweet.user.name}
                                    (@${replyTweet.user.screen_name})</label
                                >
                            </div>
                            ${mentions
                                .map((m) => {
                                    ...
                                    return html`
                                        <div
                                            class="new-tweet-mentions-modal-item"
                                        >
                                            <input
                                                type="checkbox"
                                                data-user-id="${u.id_str}"
                                                id="new-tweet-mentions-modal-item-${m}"
                                                ${this.excludeUserMentions.includes(
                                                    u.id_str
                                                )
                                                    ? ""
                                                    : " checked"}${this.user
                                            ...
                                            />
                                            <label
                                                for="new-tweet-mentions-modal-item-${m}"
                                                ${this.user.screen_name === m
                                                    ? " hidden"
                                                    : ""}
                                                >${u.name} (@${m})</label
                                            >
                            ...
                            <div style="display:inline-block;float: right;">
                                <button
                                    class="nice-button"
                                    id="new-tweet-mentions-modal-button"
                                >
                                    ${LOC.save.message}
                                </button>
                            </div>
                    ...
                    `);
                    document
                        .getElementById("new-tweet-mentions-modal-button")
                        .addEventListener("click", () => {
                            let excluded = [];
                            document
                                .querySelectorAll(
                                    '#new-tweet-mentions-modal input[type="checkbox"]'
                                )
                                .forEach((c) => {
                                    if (!c.checked)
                                        excluded.push(c.dataset.userId);
                                });
        ...
        let newTweetButton =
            document.getElementsByClassName("new-tweet-button")[0];
        let selectedIndex = 0;
            ...
            for (let index in items) {
                ...
                if (item.kind === "file") {
                    ...
                    handleFiles(
                        [file],
                        this.mediaToUpload,
                        document.getElementsByClassName("new-tweet-media-c")[0]
                    );
                }
            }
        });
        ...
        newTweetText.addEventListener("keypress", async (e) => {
            if (
                ...
                !newTweetUserSearch.hidden
            ) {
                let activeSearch = newTweetUserSearch.querySelector(
                ...
                if (!e.ctrlKey) {
                    ...
                    newTweetText.value =
                        newTweetText.value
                            ...
                            .join(" ") +
                        ` @${activeSearch
                            .querySelector(".search-result-item-screen-name")
                            .innerText.slice(1)} `;
                    if (newTweetText.value.startsWith(" "))
                        newTweetText.value = newTweetText.value.slice(1);
                    newTweetUserSearch.innerHTML = "";
                    newTweetUserSearch.hidden = true;
                }
            }
        ...
        newTweetText.addEventListener("keydown", async (e) => {
            if (e.key === "ArrowDown") {
                if (selectedIndex < newTweetUserSearch.children.length - 1) {
                    selectedIndex++;
                    newTweetUserSearch.children[selectedIndex].classList.add(
                        "search-result-item-active"
                    );
                    newTweetUserSearch.children[
                        selectedIndex - 1
                    ].classList.remove("search-result-item-active");
                } else {
                    selectedIndex = 0;
                    newTweetUserSearch.children[selectedIndex].classList.add(
                        "search-result-item-active"
                    ...
                    newTweetUserSearch.children[
                        newTweetUserSearch.children.length - 1
                    ].classList.remove("search-result-item-active");
                }
                return;
            }
            if (e.key === "ArrowUp") {
                if (selectedIndex > 0) {
                    ...
                    newTweetUserSearch.children[0].classList.remove(
                        "search-result-item-active"
                    );
                }
                return;
            ...
            if (/(?<!\w)@([\w+]{1,15}\b)$/.test(e.target.value)) {
                ...
                selectedIndex = 0;
                let users = (
                    await API.search.typeahead(
                        e.target.value.match(/@([\w+]{1,15}\b)$/)[1]
                    )
                ).users;
                newTweetUserSearch.innerHTML = "";
                users.forEach((user, index) => {
                    ...
                    if (index === 0)
                        userElement.classList.add("search-result-item-active");
                    userElement.innerHTML = html`
                        <img
                            width="16"
                            height="16"
                            class="search-result-item-avatar"
                            src="${user.default_profile_image &&
                            vars.useOldDefaultProfileImage
                                ? chrome.runtime.getURL(
                                      `images/default_profile_images/default_profile_${
                        ...
                        />
                        <span
                            class="search-result-item-name ${user.verified
                                ? "search-result-item-verified"
                                : ""}"
                            >${escapeHTML(user.name)}</span
                        >
                        <span class="search-result-item-screen-name"
                    ...
                    userElement.addEventListener("click", () => {
                        newTweetText.value =
                            newTweetText.value
                                ...
                                .split(" ")
                                .slice(0, -1)
                                .join(" ") + ` @${user.screen_name} `;
                        if (newTweetText.value.startsWith(" "))
                            newTweetText.value = newTweetText.value.slice(1);
                        if (newTweetText.value.length > 280)
                            newTweetText.value = newTweetText.value.slice(
                                0,
                                280
                            );
                        newTweetText.focus();
        ...
        newTweetText.addEventListener("input", (e) => {
            let charElement =
                document.getElementsByClassName("new-tweet-char")[0];
        ...
        document
            .getElementsByClassName("new-tweet-emojis")[0]
            .addEventListener("click", () => {
                createEmojiPicker(
                    document.getElementsByClassName("new-tweet-emojis")[0],
                    newTweetText,
                    {
                        marginLeft: "-300px",
                    }
        ...
        newTweetButton.addEventListener("click", async () => {
            ...
            if (this.excludeUserMentions.length > 0)
                tweetObject.exclude_reply_user_ids =
                    this.excludeUserMentions.join(",");
            ...
            document
                .getElementsByClassName("new-tweet-text")[0]
                .classList.remove("new-tweet-text-focused");
            document
                .getElementsByClassName("new-tweet-media-div")[0]
                .classList.remove("new-tweet-media-div-focused");
            document.getElementsByClassName(
                "new-tweet-button"
            )[0].disabled = false;
    ...
    async appendShowMore(container, data, id) {
        ...
        div.innerHTML = html`
            <button class="show-more-button center-text">
                ${data.labelText ? data.labelText : data.actionText}
            </button>
        `;
        let loading = false;
        div.querySelector(".show-more-button").addEventListener(
            "click",
            async () => {
                if (loading) return;
                loading = true;
                ...
                this.tweets = this.tweets.filter(
                    (t) => t[0] !== "showmore" || t[1].cursor !== data.cursor
                );
            }
        );
    ...
    async appendTweet(t, timelineContainer, options = {}) {
        ...
        tweet.tabIndex = -1;
        tweet.className = `tweet tweet-view ${
            options.mainTweet ? "tweet-main" : "tweet-replying"
        }`;
        if (!this.activeTweet) {
        ...
        if (t.quoted_status_id_str && !t.quoted_status && options.mainTweet) {
            ...
            try {
                if (
                    ...
                    t.quoted_status = await API.tweet.getV2(
        ...
        tweet.innerHTML = html`
            ...
            /></a>
            <div
                class="tweet-header ${options.mainTweet
                    ? "tweet-header-main"
                    : ""}"
            >
                <a
                    class="tweet-header-info ${options.mainTweet
                ...
                >
                    <b
                        ${t.user.id_str === "1708130407663759360"
                            ...
                            : ""}class="tweet-header-name ${options.mainTweet
                    ...
                    >
                    <span class="tweet-header-handle"
                        >@${t.user.screen_name}</span
                    >
                </a>
                ${options.mainTweet && t.user.id_str !== user.id_str
                    ? `<button class='nice-button tweet-header-follow ${
            ...
            <a
                ...
                class="tweet-time"
                data-timestamp="${new Date(t.created_at).getTime()}"
            ...
            >
            <article
                class="tweet-body ${options.mainTweet ? "tweet-body-main" : ""}"
            >
                <div
                    lang="${t.lang}"
                ...
                >
                    <span class="tweet-body-text-span"
                        >${vars.useOldStyleReply
                            ? /*html*/ mentionedUserText
                            : ""}${full_text
                            ? await renderTweetBodyHTML(t)
                            : ""}</span
                    >
                </div>
                ${!isMatchingLanguage
                ...
                ${t.extended_entities && t.extended_entities.media
                    ? html`
                          ...
                          ${t.extended_entities &&
                          t.extended_entities.media &&
                          t.extended_entities.media.some(
                          ...
                          ${videos
                              ? html`
                                    <div class="tweet-media-controls">
                                        ${videos[0].ext &&
                                        videos[0].ext.mediaStats &&
                                        videos[0].ext.mediaStats.r &&
                                        videos[0].ext.mediaStats.r.ok
                                            ? `<span class="tweet-video-views">${Number(
                                                  videos[0].ext.mediaStats.r.ok
                                                      .viewCount
                                              )
                                                  .toLocaleString()
                ...
                ${t.quoted_status
                    ? html`
                          <a
                              ...
                              href="/${t.quoted_status.user
                                  .screen_name}/status/${t.quoted_status
                                  .id_str}"
                          >
                              <img
                                  src="${t.quoted_status.user
                                                ...
                                                .map(
                                                    (m) =>
                                                        `<${
                                                            ...
                                                            m.type ===
                                                            "animated_gif"
                                                                ? "disableRemotePlayback loop muted"
                                                                : ""
                                                        }${
                                                            m.type ===
                                                                "animated_gif" &&
                              ...
                              ${!isQuoteMatchingLanguage
                                  ? html`
                                        <span
                                            class="tweet-button tweet-quote-translate"
                                            >${LOC.view_translation
                                                .message}</span
                                        >
                                    `
                                  : ``}
                ...
                `.replace(
                          ...
                          tweet.trusted_circle_owner
                              ...
                              ? tweetStorage[t.conversation_id_str].user
                                    .screen_name
                ...
                ${(t.withheld_in_countries &&
                    (t.withheld_in_countries.includes("XX") ||
                        t.withheld_in_countries.includes("XY"))) ||
                t.withheld_scope
                    ? `<div class="tweet-warning">This Tweet has been withheld in response to a report from the copyright holder. <a href="https://help.twitter.com/en/rules-and-policies/copyright-policy" target="_blank">Learn more.</a></div>`
                    : ""}
                    ...
                    ? `<div class="tweet-warning">${
                          ...
                          (t.user.id_str === user.id_str ||
                              ...
                              (t.conversation_control.policy.toLowerCase() ===
                                  "by_invitation" &&
                                  full_text &&
                                  full_text.includes(`@${user.screen_name}`)))
                    ...
                    ? html`
                          <div class="tweet-footer">
                              <div class="tweet-footer-stats">
                                  ...
                                  t.quote_count > 0 /*html*/
                                      ? `<a href="/${
                                            t.user.screen_name
                                        }/status/${
                                            t.id_str
                                        }/retweets/with_comments" class="tweet-footer-stat tweet-footer-stat-q">
                            <span class="tweet-footer-stat-text">${
                                LOC.quotes.message
                            }</span>
                            <b class="tweet-footer-stat-count tweet-footer-stat-quotes">${formatLargeNumber(
                                t.quote_count
                            ).replace(/\s/g, ",")}</b>
                        </a>`
                                      : ""}
                                  <a
                                      href="/${t.user
                                          .screen_name}/status/${t.id_str}/likes"
                ...
                <a
                    ...
                     ・ ${t.source
                        ? t.source.split(">")[1].split("<")[0]
                        : "Unknown"}</a
                >
                <div class="tweet-interact">
                    <span
                        class="tweet-button tweet-interact-reply"
                        title="${LOC.reply_btn.message}${!vars.disableHotkeys
                            ? " (R)"
                            : ""}"
                        data-val="${t.reply_count}"
                        >${options.mainTweet
                            ? ""
                    ...
                    <span
                        ...
                        class="tweet-button tweet-interact-retweet${t.retweeted
                    ...
                    >
                        <span class="tweet-interact-retweet-menu-retweet"
                            >${t.retweeted
                                ...
                                : LOC.retweet.message}</span
                        >
                        <span class="tweet-interact-retweet-menu-quote"
                            >${LOC.quote_tweet.message}</span
                        >
                        ${options.mainTweet
                            ? html`
                                  <span
                    ...
                    <span
                        title="${vars.heartsNotStars
                            ? LOC.like_btn.message
                            : LOC.favorite_btn.message}${!vars.disableHotkeys
                            ? " (L)"
                            : ""}"
                        class="tweet-button tweet-interact-favorite ${t.favorited
                    ...
                    <div class="tweet-interact-more-menu dropdown-menu" hidden>
                        ...
                        ${t.user.id_str === user.id_str
                            ? html`
                                  ...
                                  >
                                  <span class="tweet-interact-more-menu-delete"
                        ...
                        t.user.id_str !== user.id_str
                            ? html`
                                  <span class="tweet-interact-more-menu-hide"
                                      >${t.moderated
                ...
                <div class="tweet-edit-section tweet-reply" hidden>
                    ...
                    >
                        ${LOC.reply.message}</button
                    ><br />
                    <span class="tweet-reply-char"
                        >${localStorage.OTisBlueVerified
                            ? "0/25000"
                            : "0/280"}</span
                    ><br />
                    <div
                ...
                <div
                    class="tweet-view-self-thread-div"
                    ${options.threadContinuation ? "" : "hidden"}
                >
        ...
        if (options.mainTweet) {
            let likers =
                ...
                typeof t.quote_count !== "undefined" &&
                t.quote_count > 0
                    ? this.mainTweetLikers.slice(0, 6)
                    : this.mainTweetLikers.slice(0, 8);
            for (let i in likers) {
                let liker = likers[i];
        ...
        if (
            t.birdwatch &&
            t.birdwatch.subtitle &&
            options.mainTweet &&
            !vars.hideCommunityNotes
        ) {
            let div = document.createElement("div");
        ...
        tweetInteractRetweetMenuRetweet.addEventListener("click", async () => {
            if (!t.retweeted) {
                ...
                tweetInteractRetweetMenuRetweet.innerText =
                    LOC.unretweet.message;
        ...
        tweetQuoteButton.addEventListener("click", async () => {
            ...
            if (!tweetData) {
                tweetQuoteError.innerHTML = html`${LOC.error_sending_tweet
                        .message}<br />`;
                tweetQuoteButton.disabled = false;
                return;
            }
        ...
        tweetInteractFavorite.addEventListener("click", () => {
            if (t.favorited) {
                ...
                if (!options.mainTweet) {
                    ...
                    if (
                        this.mainTweetLikers.find(
                            (liker) => liker.id_str === user.id_str
                        )
                    ) {
                        this.mainTweetLikers.splice(
                            this.mainTweetLikers.findIndex(
                                (liker) => liker.id_str === user.id_str
                            ),
                            1
                        );
                ...
                if (t.user.following) {
                    ...
                    if (LOC.follow_user.message.includes("$SCREEN_NAME$")) {
                        tweetInteractMoreMenuFollow.innerText =
                            LOC.follow_user.message.replace(
                                "$SCREEN_NAME$",
                                t.user.screen_name
                            );
                    } else {
                    ...
                    if (LOC.unfollow_user.message.includes("$SCREEN_NAME$")) {
                        tweetInteractMoreMenuFollow.innerText =
                            LOC.unfollow_user.message.replace(
                                "$SCREEN_NAME$",
                                t.user.screen_name
                            );
                    } else {
        ...
        tweetInteractMoreMenuNewtwitter.addEventListener("click", () => {
            openInNewTab(
                `https://${location.hostname}/${t.user.screen_name}/status/${t.id_str}?newtwitter=true`
            );
        });
        ...
        if (t.user.id_str === user.id_str) {
            tweetInteractMoreMenuAnalytics.addEventListener("click", () => {
                openInNewTab(
                    `/${t.user.screen_name}/status/${t.id_str}/analytics?newtwitter=true`
                );
            });
            tweetInteractMoreMenuDelete.addEventListener("click", async () => {
                ...
                if (options.mainTweet) {
                    let tweets = Array.from(
                        timelineContainer.getElementsByClassName("tweet")
                    );
                    if (tweets.length === 0) {
                        document
    ...
    async popstateChange(that) {
        ...
        if (
            ...
            that.subpage === "retweets_with_comments" &&
            document.getElementById("this-is-tweet-page")
        ) {
    ...
    init() {
        document
            .getElementsByClassName("timeline-more")[0]
            .addEventListener("click", async (e) => {
        ...
        document
            .getElementsByClassName("likes-more")[0]
            .addEventListener("click", async () => {
        ...
        document
            .getElementsByClassName("retweets-more")[0]
            .addEventListener("click", async () => {
        ...
        document
            .getElementsByClassName("retweets_with_comments-more")[0]
            .addEventListener("click", async (e) => {
                e.target.innerText = LOC.loading_tweets.message;
        ...
        this.container.addEventListener("scroll", this.scrollHelper, {
            passive: true,
        });
    }
    close() {
